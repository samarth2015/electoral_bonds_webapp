...
def main_page():
    if "unique_individual" in session:
        ...
        return render_template("index.html", name_of_purchaser = unique_individual, name_of_political_party = unique_party, pairs = party_purchaser)
    else:
...
def search_by_individual_per_year():
    if request.method == 'POST':
        ...
        for i in range(2019, 2025):
            cursor.execute("select Denominations from individual where `Name of the Purchaser` = %s and `Journal Date` like %s", (individual, '%' + str(i)))
            data2 = list(cursor.fetchall())
...
def search_by_political_party_individual():
    if request.method == "POST":
        political_party = request.form.get('political_party')
        individual = request.form.get('individual')
        if political_party == 'default' and individual == 'default':
            return main_page()
        elif individual == 'default':
            cursor = mysql.connection.cursor()
            cursor.execute('select `Name of the Purchaser`, individual.Denominations from individual inner join political_party on individual.`Bond Number` = political_party.`Bond Number` and substring(`Journal Date`, 8, 4) = substring(`Date of Encashment`, 8, 4) where `Name of the Political Party` = %s', (political_party,))
            ...
            cursor = mysql.connection.cursor()
            cursor.execute('select individual.Denominations from individual inner join political_party on individual.`Bond Number` = political_party.`Bond Number` and substring(`Journal Date`, 8, 4) = substring(`Date of Encashment`, 8, 4) where `Name of the Purchaser` = %s and `Name of the Political Party` = %s', (individual, political_party))
            data_political = cursor.fetchall()
